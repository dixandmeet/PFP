...
export default function PlayerReportsPage() {
  ...
  const handleShareReport = async (reportId: string) => {
    try {
      ...
      const slug = report?.shareSlug || reportId
      const url = `${window.location.origin}/shared/reports/${slug}`
  ...
  const handleViewReport = (reportId: string) => {
    router.push(`/player/reports/${reportId}`)
  }

  const handleEditReport = (reportId: string) => {
    router.push(`/player/reports/${reportId}/edit`)
  }
