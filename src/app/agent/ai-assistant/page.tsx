...
export default function AgentAIAssistantPage() {
  ...
  const onSubmit = async (data: { message: string }) => {
    ...
    const userMessage: Message = { role: "user", content: data.message, timestamp: new Date() }
    ...
    try {
      ...
      if (result.type === "text_response") {
        setMessages([...messages, userMessage, { role: "assistant", content: result.content, timestamp: new Date() }])
      } else if (result.type === "tool_call_proposal") {
        setMessages([...messages, userMessage, { role: "assistant", content: result.preview, timestamp: new Date() }])
        setPendingAction(result)
        setConfirmDialogOpen(true)
      }
    } catch (error: any) {
      toast({ title: "Erreur", description: error.message, variant: "destructive" })
      setMessages([...messages, userMessage, { role: "assistant", content: "Erreur.", timestamp: new Date() }])
    } finally {
      setLoading(false)
    }
  }
  ...
  const confirmAction = async () => {
    ...
    try {
      const response = await fetch("/api/ai/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ pendingActionId: pendingAction.pendingActionId }),
      })
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <div className="grid gap-2 md:grid-cols-2">
              <Button variant="outline" className="justify-start">Génère une shortlist de joueurs</Button>
              <Button variant="outline" className="justify-start">Trouve des opportunités pour mes joueurs</Button>
              <Button variant="outline" className="justify-start">Prépare une soumission</Button>
              <Button variant="outline" className="justify-start">Rédige un message de relance</Button>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>L'assistant souhaite effectuer cette action</DialogDescription>
          </DialogHeader>
