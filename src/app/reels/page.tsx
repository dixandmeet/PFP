...
export default function ReelsPage() {
  ...
  const loadReels = useCallback(async (pageNum: number) => {
    try {
      const response = await fetch(`/api/posts/reels?page=${pageNum}&limit=10`)
  ...
  useEffect(() => {
    ...
    reels.forEach(reel => {
      if (!likes.hasOwnProperty(reel.id)) {
    ...
    setLikedReels(prev => ({ ...prev, ...likes }))
    setBookmarkedReels(prev => ({ ...prev, ...bookmarks }))
    setLikeCounts(prev => ({ ...prev, ...counts }))
    setCommentCounts(prev => ({ ...prev, ...commentC }))
    setShareCounts(prev => ({ ...prev, ...shareC }))
  }, [reels])
  ...
  }, [currentIndex, reels.length, isTransitioning, hasMore, page, loadReels])
  ...
  const getUserDisplayName = (user: ReelUser) => {
    ...
    if (user.agentProfile) return `${user.agentProfile.firstName} ${user.agentProfile.lastName}`
    if (user.clubProfile) return user.clubProfile.clubName
    return user.email
  }

  const getUserAvatar = (user: ReelUser) => {
    return user.playerProfile?.profilePicture || user.agentProfile?.profilePicture || user.clubProfile?.logo
  }
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-pitch-500 to-pitch-600 flex items-center justify-center">
            <Loader2 className="h-8 w-8 text-white animate-spin" />
          </div>
          <p className="text-white/70 font-medium">Chargement des Reels...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center">
        <div className="flex flex-col items-center gap-6 text-center px-6">
          ...
          <div>
            <h2 className="text-white text-xl font-bold mb-2">Aucun Reel disponible</h2>
          ...
          <div className="absolute right-3 bottom-32 flex flex-col items-center gap-5 z-20">
            <button onClick={(e) => { e.stopPropagation(); handleLike() }} className="flex flex-col items-center gap-1 group">
              <div className={cn(
                ...
                likedReels[currentReel.id] ? "bg-red-500/20" : "bg-white/10 group-hover:bg-white/20"
              )}>
                <Heart className={cn(
                  "h-6 w-6 transition-all",
                  likedReels[currentReel.id] ? "text-red-500 fill-red-500 scale-110" : "text-white"
                )} />
              </div>
            ...
            <button onClick={(e) => { e.stopPropagation(); handleBookmark() }} className="flex flex-col items-center gap-1 group">
              <div className={cn(
                "p-2.5 rounded-full transition-all",
                bookmarkedReels[currentReel.id] ? "bg-gold-500/20" : "bg-white/10 group-hover:bg-white/20"
              )}>
                <Bookmark className={cn(
                  "h-6 w-6 transition-all",
                  bookmarkedReels[currentReel.id] ? "text-gold-500 fill-gold-500" : "text-white"
                )} />
              </div>
            </button>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    actualIndex === currentIndex ? "h-6 bg-white" : "h-2 bg-white/30"
                  )}
                />
              )
            })}
